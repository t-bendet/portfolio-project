...
`architecture-decision.md` §1–6; `phase2-scaffold-plan.md`;
...
| Inline theme script + keydown buffer | ADR 0002, tokens-reference §2 | Runs on every public page; observes all keystrokes on pages that contain **no input fields** (no forms exist anywhere in the public IA — contact is `mailto:` only) |
...
- **SR-13. Instance security group: 443/80 open; SSH restricted to Tal's
  ...
  If SSH: key-only auth, no password auth. *(Verify: provisioning
  checkpoint — phase2-workflow §7.2.)*
...
- **SR-24. The incantation's shipped form is decided deliberately** at
  ...
  identifier vocabulary (tokens-reference §1) hold around it either way.
