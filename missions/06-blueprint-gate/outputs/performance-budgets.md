...
`typography-spec.md` (with coherence C3's correction: the warm theme is
**4** families, not 3); `tokens-reference.md` §2; `content-model.md` §6;
...
motion limited to color transitions (motion-and-texture §1). The budgets
...
is built on. Lazy strategy is Phase 2's choice within typography-spec §9's
...
- `size-adjust`/`ascent-override` tuning for the Hebrew companions is owed
  (typography-spec §3) — it is also the CLS insurance for mixed-script
...
- No other imagery exists by design (no og-image decision yet —
  navigation-spec §3.4 flags it; when made, the asset is build-time static
