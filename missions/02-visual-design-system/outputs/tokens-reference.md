...
Basis: `palette-spec.md` (v2, prototype-exact — normative for every color
value), ADR 0002 (active — fixes the switching mechanism), the design-tokens
skill's delivery format, ADR 0011 (RTL). Framework-agnostic by construction:
ADR 0003 is reopened, so this spec commits only to plain CSS custom
properties — a translation layer (e.g. into a framework's theme config) is
Phase 2 work under whatever Mission 3 decides.
...
| Second theme selector | `[data-theme="warm"]` | **Final naming decision:** `warm` — mundane, describes temperature, points at nothing. The palette-spec placeholder is hereby confirmed. |
...
| Transition | 600ms on `background-color`, `color`, `border-color`, `fill` | ADR 0002 fixes 600ms; `motion-and-texture.md` §3 defines easing and the `prefers-reduced-motion` behavior (instant switch) |
| Console log on toggle | ADR 0002's | untouched |

**Total-coverage rule** (palette-spec §7.7, restated as the switching
...
Color values are normative in `palette-spec.md` §2/§3 (provenance and AA
...
### 3.5 Typography tokens (names here, values in `typography-spec.md`)
...
1. Every `--*` color/type token appears in both theme blocks (value or
   ...
   palette-spec §10.)
2. Contrast gate: `scripts/contrast.ts --pairs` over palette-spec §5's pair
...
palette-spec §8 as a registry-level rule so it survives into component
specs.
