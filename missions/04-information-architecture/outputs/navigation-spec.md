...
Basis (law): `sitemap.md` (hardened — route set, the mark-as-home-link entry,
the Hebrew seam consequences, the `/404` constraint), `content-model.md`
(hardened — feeds, RTL contract, dynamic-layer touchpoints); ADRs 0001
(mark + eyebrow), 0002 (hidden theme), 0011 (RTL), 0018 (portrait scope),
0019/0020 (static core, dynamic layer);
`missions/01-*/outputs/symbol-and-language-map.md` (allowed/banned vocabulary
with tests); `missions/02-*/outputs/typography-spec.md` §6–7 (eyebrow
treatment, RTL type rules), `palette-spec.md` §7–8 (accent placement, logical
edges), `motion-and-texture.md` §1 (motion budget).
...
are in `page-briefs/`. No pixel values, no finished copy.
...
when you arrive (`page-briefs/writing-index.md` §4), rather than being hidden
from the nav.
...
`typography-spec.md` §6 fixes the eyebrow's *treatment* (mono family,
...
1. **The mark segment `T://bendet` is never translated, transliterated,
   restyled, or themed** (ADR 0001; `typography-spec.md` §6). It renders
...
3. **Linking the mark must not alter its resting appearance.** No persistent
   ...
   (`palette-spec.md` §7.5). Test: screenshot the eyebrow on `/about/` and on
...
4. **Hebrew segments carry no `text-transform` and `letter-spacing: normal`**
   (`typography-spec.md` §6 — Hebrew is unicameral; 0.2em tracking is a
...
- **Open, and deliberately not decided by M4:** social-card imagery (OG /
  ...
  and no composition in `page-briefs/` depends on it. Flagged for Phase 2 so
...
2. **All chrome geometry binds logically** — `border-inline-start`,
   ...
   (`tokens-reference.md` §5, `palette-spec.md` §8.1). The nav item's current
