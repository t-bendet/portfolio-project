...
ADRs 0011, 0018, 0019, 0020 · `typography-spec.md` §7 (code is LTR) ·
`palette-spec.md` §7.3 (callout idiom per temperature) ·
`motion-and-texture.md` §1 (motion budget).
...
- **Accents are never backgrounds**, and **links are `--text` with an
  ...
  `--link` token (`palette-spec.md` §7.1, §7.4). Long-form prose is where the
...
- **No scroll-spy.** An active-section indicator requires a scroll listener —
  ...
  (`motion-and-texture.md` §1: "nothing moves on load, on scroll, or on a
...
| A "back to top" control | `navigation-spec.md` §4.2. |
...
- **Sidebar carrying site navigation instead of in-page contents.** Rejected:
  ...
  (`navigation-spec.md` §1); a second navigation surface on one archetype
