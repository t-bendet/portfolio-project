...
`palette-spec.md` §1 (overview archetype), §7 (accent placement).
...
2. **The section's terms**, in the callout idiom the archetypes already own
   ...
   `--badge-N` tint fill — `palette-spec.md` §7.3). Its content: what belongs
...
- **Hiding the `writing` nav item while the section is empty.** Rejected —
  `navigation-spec.md` INV-3: chrome shape must not track content volume, and
