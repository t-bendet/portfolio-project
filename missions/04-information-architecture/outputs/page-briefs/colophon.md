...
| `lang` / `dir` | `en` / `ltr` |
| Dynamic layer | **none, and this is load-bearing** — it must not display live build or deploy status |
...
obligation handed to M5) · ADR 0012 · `architecture-decision.md` §1–6 ·
...
| 8 | The pipeline | what "from scratch" honestly means — the vendor's workflow syntax and primitive actions, every stage's logic authored (`architecture-decision.md` §5). Paired cost: ARM builds via QEMU on free runners, and the time that costs | always |
...
| 11 | Footer | per `navigation-spec.md` §4 | always |
...
The page links the public repository and, inside it, `docs/decisions/`. This
is the strongest available version of the page's claim: rather than asserting
that the choices were considered, it points at the record where they were
argued, including the ones that were rejected and the ones that were
reversed. It also keeps this page short — the colophon summarizes, the ADRs
are the depth.
