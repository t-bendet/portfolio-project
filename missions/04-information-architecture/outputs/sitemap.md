...
**Status: hardened.** Checkpoint 1 happened 2026-07-21: Tal decided all three
open calls (§5). Everything else follows mechanically from active ADRs.

Basis (law): ADRs 0001, 0002, 0011, 0012, 0013, 0014, 0015–0018 (M2),
0019–0021 (M3). Input only (reopened, this mission's to resolve): 0009, 0010.
Identity law: `missions/01-*/outputs/identity-thesis.md`,
`symbol-and-language-map.md`. Structural law:
`missions/02-*/outputs/palette-spec.md` §1 (archetypes),
`tokens-reference.md` §2 (theme switching),
`missions/03-*/outputs/architecture-decision.md` §1 (static/dynamic boundary).
...
1. **It un-hides the hidden theme.** ADR 0002 ships *one* dark design to all
   ...
   routes.
2. **It breaks the switching mechanism as specified.** `tokens-reference.md`
...
- **Chrome language:** Hebrew. This page's own nav, labels, and eyebrow
  ...
  translated or transliterated — ADR 0001, typography-spec §6).
...
- **hreflang honesty (correcting an assumption carried from M3):**
  `architecture-decision.md` §1 notes hreflang alternates via the sitemap
...
- **Excluded from:** primary nav, footer, sitemap.xml, and RSS. `robots.txt`
  ...
  words), and it is out of scope for `page-briefs/`.
...
1. Navigation inside the Hebrew subtree links to English pages for
   everything except the translations. `navigation-spec.md` must handle this
...
Already out of scope per `architecture-decision.md` §7. At launch's corpus
