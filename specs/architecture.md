...
minutes are free. ARM64 (verified 2026-07-22): GitHub's hosted native arm64
runners (`ubuntu-24.04-arm`) are GA and free for public repos — arm64 images
build natively there; QEMU/buildx cross-emulation is not used
(`scaffold-plan.md` §0).
...
6. **migrate** — `prisma migrate deploy` against the production DB before
   ...
   unverified; re-verify at scaffold — see scaffold-plan.md).
...
- **Budget (verified 2026-07-22):** ≈ $10.28/mo — instance $6.13 + hosted
  zone $0.50 + public IPv4 $3.65 (confirmed, assume no free-tier
  exemption) + S3 backup pennies. Inside the $15 ceiling with ~$4.70
  headroom. Region pricing re-verified at provisioning (`scaffold-plan.md`
  §6).
...
The scaffold-time verifications this section once listed as open all ran on
2026-07-22; results (pins, native ARM runners, IPv4 billing → ≈$10.28/mo
budget) are merged into `scaffold-plan.md` §0.
