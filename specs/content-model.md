...
Decided 2026-07-21; Tal decided all open calls in §4 and supplied the
upstream translation terms that govern most of that collection (§4.0).

Basis: `routes/sitemap.md`; `architecture.md` §1–2; `design/typography.md`
§6–7 (RTL type rules).
...
- **Code blocks stay `dir="ltr"`** inside the RTL page
  (typography.md §7.1) — code is notation, not prose.
- **The `T://bendet` mark is never translated or transliterated** in the
  eyebrow or anywhere else (ADR 0001, typography.md §6). The section
...
2. **Exception — code.** `pre`/`code` carry `dir="ltr"` unconditionally
   (typography.md §7.1). Inline Hebrew inside a comment or string renders
...
3. **Exception — the mark.** `T://bendet` is an LTR run inside RTL prose;
   ...
   `://` misordered (typography.md §6).
4. **No physical-side CSS anywhere in content rendering.** Logical
   properties only — `border-inline-start`, `padding-inline`,
   `margin-inline`, `text-align: start` (tokens.md §5,
   palette.md §8). This is already law at the token layer; restated here
...
5. **Hebrew eyebrow segments carry no `text-transform` and
   ...
   a Latin-caps idiom (typography.md §6).
...
`localStorage` persistence, and `tokens.md` §2 (M2, closed, law)
...
3. **hreflang and path-keying traps (resolved in the merged specs).**
   `scaffold-plan.md` §2 now states directly: configure `@astrojs/sitemap`
   **without** its `i18n` option — no hreflang alternates. Likewise view
   events are keyed by the namespaced analytics key (§6), never by raw
   path. Both rules were once split across decision records; they are
