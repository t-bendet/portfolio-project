...
separate tokens (tokens.md §3.5). The browser routes Hebrew
...
- Mono family (`--font-mono`), `--size-eyebrow` (dark) / `--size-label`
  ...
  subtle/muted eyebrow values are AA-fixed by palette.md §4).
...
families; per-theme payload is **3 families dark / 4 families warm**
(corrected at the blueprint gate — the original count of 3 for warm was a
miscount; `performance-budgets.md` §4.2 carries the corrected accounting.
Count warm as 4 when sizing any font lazy-loading) (the hidden theme's fonts load
