...
2. **Code panels carry `dir="ltr"` unconditionally** — code is notation, not
   prose (rule 2, `typography.md` §7.1). Hebrew inside a comment or
...
9. **Italic is unavailable in the dark temperature and unavailable for Hebrew
   in either** (`typography.md` §5). No meaning may hang on italics on
...
Hebrew link — `navigation.md` §4.1, invariant R-3). Two honest
consequences:
