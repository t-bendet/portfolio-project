...
| Dynamic layer | **none** — no view event, no counts, nothing; no dynamic-layer script (the only client script on this route is the global theme script — ADR 0002, `tokens.md` §2). The 404 beacon was considered and rejected on data-model grounds (`content-model.md` §6) |
| Chrome | per `navigation.md`; eyebrow `T://bendet · 404`; **no nav item is current** |

Basis (law): `sitemap.md` §1 (canonical trailing-slash form), §2 row 11 ·
`navigation.md` §7.3 (the chrome-vs-body conflict, resolved) · ADR 0002 +
`symbol-and-language-map.md` banned #7 (labelled easter eggs) ·
`architecture.md` §4 (Caddy serves the static bundle).
...
(`architecture.md` §4) recorded here because it is an IA property —
...
**Resolution (also recorded in `navigation.md` §7.3): row 11 governs the
...
3. The Hebrew safeguard exists for visitors who have lost their way, which is
   ...
   their one-click way back (`navigation.md` invariant R-3).
...
| Jokes, ASCII art, animations | `hero-and-motion.md` §1's budget has no slot, and the visitor is mildly annoyed already. |
