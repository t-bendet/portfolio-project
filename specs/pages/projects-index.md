...
| Chrome | per `navigation.md`; eyebrow `T://bendet · projects` |

Basis (law): `sitemap.md` §2 rows 6–7 · `content-model.md` §3.3, §6 ·
`palette.md` §1 (overview archetype), §7.2 (chip badges) ·
...
| 6 | Footer | per `navigation.md` §4 | always |
...
| Stack chips | `stack[]` | non-linking chips, `--badge-N` fills behind `--accent-N` text (`palette.md` §7.2). Not links — there is no stack-index route |
...
(`palette.md` §1's card-grid pattern is a grid, not a stretch).

### 2.4 What this page does not carry

| Cut | Reason |
|---|---|
| Screenshots / device mockups | Neither prototype contains an image treatment, `hero-and-motion.md` §2 bans shadow and frame ornament, and a screenshot of a web app is the most template-shaped element available. If a project needs a visual, it belongs inside its case study. |
