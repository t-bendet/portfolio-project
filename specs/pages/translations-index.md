...
| Chrome language | Hebrew, except the mark and the four English nav labels (`navigation.md` §2.2) |
...
`typography.md` §6–7 (Hebrew eyebrow, RTL rules).
...
| 4 | Scope pointer | one line: Tal's own writing is in English at `/writing/`, labelled in English per the label rule (`navigation.md` §2.2). This is the seam, shown | always |
...
| 7 | Footer | `/he/rss.xml`, colophon, direct links, the Hebrew link — **not** marked current here, even though this is its destination: the nav occurrence carries `aria-current`, the footer duplicate carries nothing (`navigation.md` §2.3, test N-4) | always |
...
- The Hebrew eyebrow segment: no `text-transform`, `letter-spacing: normal`
  (`typography.md` §6). The mark segment is unchanged in every respect.
- Hebrew renders through the companion faces in the stacks (Heebo dark /
  Frank Ruhl Libre warm) — `typography.md` §3. No separate tokens.
...
2. **A statement of the section's terms**, in Hebrew, in the callout idiom
   (`palette.md` §7.3): what gets translated here (technical writing
